...
def parse_date_str(date_str: str) -> Optional[datetime.date]:
    ...
    if match_full:
        try:
            return datetime.date(int(match_full.group(1)), int(match_full.group(2)), int(match_full.group(3)))
        except ValueError:
            return None
    ...
    if match_short:
        try:
            return datetime.date(today.year, int(match_short.group(1)), int(match_short.group(2)))
        except ValueError:
            return None
...
def calculate_overlap(session_start: float, session_end: float, window_start: float, window_end: float) -> int:
